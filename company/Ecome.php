...
class Ecome
{
    ...
    private static $_env = [
        'test' => [//测试环境(暂时没有s)
            ...
            'getPinCodes' => 'http://staging.ecomexpress.in/apiv2/pincodes/',            //获取pin code
            'getAwb' => 'http://staging.ecomexpress.in/apiv2/fetch_awb/',                //获取Awb code
            'createOrder'  => 'http://staging.ecomexpress.in/apiv2/manifest_awb/',       //创建订单(COD/PPD订单)
            'createOrder2'  => 'http://staging.ecomexpress.in/apiv2/manifest_awb_rev_v2/',//创建订单2 适用于REV(反向)订单
            'trackOrder' => 'http://staging.ecomexpress.in/track_me/api/mawbd/',         //Status Pull
        ],
        'product' => [//正式环境(暂时没有s) 注意:必须提供ip给Ecome公司,才可以请求(正式下)
            'username' => 'arayna841670',//ARAYNA IMPEX PRIVATE LIMITED - 841670
            'password' => 'ara07yn16im84na',
            'getPinCodes' => 'http://api.ecomexpress.in/apiv2/pincodes/',
            'getAwb' => 'http://api.ecomexpress.in/apiv2/fetch_awb/',
            'createOrder'  => 'http://api.ecomexpress.in/apiv3/manifest_awb/',
            'createOrder2'  => 'http://api.ecomexpress.in/apiv2/manifest_awb_rev_v2/',
            'trackOrder' => 'https://plapi.ecomexpress.in/track_me/api/mawbd/',
        ],
    ];
    /*url : https://cp.ecomexpress.in/#/login
    customer : ARAYNA IMPEX PRIVATE LIMITED
    username : 841670
    password :*/

    //alljoy 印度海外仓地址
    private static $_warehouse = [
        'name' => 'ALLJOY SUPPLY CHAIN (INDIA) PVT LTD.',
        'address_line1' => 'Khasra NO 72 & 73 Extended Laldoora Of Village Bamnoli Sector 28 Dwarka New Delhi 110077',
        'pincode' => '110077',
        'phone' => '9667030414',
        'mobile' => '9667030414',
    ];
    ...
    private static $_GST = [
        'SELLER_GSTIN' => '09BDQPS4397J1ZH',
        'GST_TAX_NAME' => '0517523973',
    ];
    /* 下单说明:
    getPinCode  --> pin code (一周更新一次,一周内请求的pin code一样,请求一次(固定值)1502个pin code) [建议存数据库然后定期请求(文档描述)]
       getAwb   --> Awb number(一次请求可设置获取数量,每次请求的Awb不一样) [建议存数据库然后定期请求(文档描述)]
    下单 = pin code(必填) + Awb number(必填) + order number(必填)
    查询 = Awb number(必填) + order number(选填)
     * */

    /*下单1 此方式适用于PPD(预付)/COD(货到)订单
    *@return array $result keys:
     * status : success | failed
     * trackingNo : 707xxx (当 status = 'success')
     * msg : error message (当 status = 'failed')
     * */
